...
Better Monitor 把进程、网络、端口、登录项和硬件状态放进同一个窗口，适合日常查看，也适合有针对性地排查问题。本说明对应 1.0.2 版本，按界面从上到下逐页介绍。
...
7. [端口与服务](#端口与服务)
8. [启动项](#启动项)
9. [AI 进程总结与 AI 历史](#ai-进程总结与-ai-历史)
10. [菜单栏](#菜单栏)
11. [设置](#设置)
12. [快捷指令](#快捷指令)
13. [隐私与本地数据](#隐私与本地数据)
...
或者从 [GitHub Releases](https://github.com/BetterMacNet/better-monitor/releases/latest) 下载 `BetterMonitor-1.0.2.dmg`，把 `Better Monitor.app` 拖进「应用程序」文件夹。每个发布版本都用 Developer ID 签名并经过 Apple 公证，首次打开不需要联网验证。
...
- **侧边栏**：监控概览、进程监控、网络、端口与服务、启动项、AI 历史、设置七个页面。底部卡片显示本机型号、macOS 版本和开机时长。
