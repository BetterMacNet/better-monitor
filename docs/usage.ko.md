...
Better Monitor는 프로세스, 네트워크, 포트, 로그인 항목, 하드웨어 상태를 하나의 창에 모은 macOS용 시스템 모니터입니다. 평소 상태를 확인할 때도, 특정 문제를 추적할 때도 쓸 수 있습니다. 이 설명서는 1.0.3 버전을 기준으로 화면 순서대로 안내합니다.
...
8. [포트 및 서비스](#포트-및-서비스)
9. [로그인 항목](#로그인-항목)
10. [AI 프로세스 요약과 AI 기록](#ai-프로세스-요약과-ai-기록)
11. [메뉴 막대](#메뉴-막대)
12. [설정](#설정)
13. [단축어](#단축어)
14. [개인정보 보호와 로컬 데이터](#개인정보-보호와-로컬-데이터)
...
또는 [GitHub Releases](https://github.com/BetterMacNet/better-monitor/releases/latest)에서 `BetterMonitor-1.0.3.dmg`를 내려받아 `Better Monitor.app`을 「응용 프로그램」 폴더로 드래그하세요. 모든 릴리스는 Developer ID로 서명되고 Apple의 공증을 받았으므로 처음 열 때 네트워크 확인이 필요하지 않습니다.
...
- **사이드바**: 모니터링 개요, AI 모니터, 프로세스 모니터링, 네트워크, 포트 및 서비스, 로그인 항목, AI 기록, 설정의 8개 화면. 아래쪽 카드에 Mac 모델, macOS 버전, 가동 시간이 표시됩니다.
...
AI 모니터에서는 Apple 실리콘이 지금 무엇을 하고 있는지, 로컬 AI 모델이 어디에서 실행되는지 자세히 볼 수 있습니다. 사이드바의 「모니터링 개요」 아래에 있으며 Apple 실리콘 Mac이 필요합니다.

![AI 모니터](../screenshots/ko/14-ai-monitor.webp)

- **AI 워크로드**: 현재 작업이 무엇인지 한 줄로 판정합니다(GPU(Metal)의 LLM, Neural Engine의 Core ML, 미디어 엔진의 동영상, 유휴). CPU, GPU, Neural Engine, 미디어 엔진, 메모리 각각의 상태와 값도 함께 보여 줍니다.
- **AI 런타임**: Ollama, LM Studio, MLX, llama.cpp 같은 로컬 런타임을 프로세스로 감지하고, 메모리·CPU 사용량과 모델 예산(메모리에 들어갈 수 있는 가장 큰 모델)을 표시합니다.
- **런타임 프로세스** 및 **로컬 AI 런타임 API**: 감지된 모든 런타임 프로세스를 보여 줍니다. 「로컬 AI 런타임에 연결」을 켜면 127.0.0.1의 런타임 API에서 로드된 모델, GPU/CPU 분배, 초당 토큰 수를 읽어옵니다. 데이터는 Mac 밖으로 전송되지 않습니다. Better Monitor는 런타임을 실행하거나 포트를 스캔하지 않습니다.
- **CPU** 및 **GPU / 미디어 / Neural Engine**: 효율 코어와 성능 코어의 사용률과 클럭, GPU 사용률·전력·메모리, Neural Engine 전력, 미디어 엔진 대역폭을 각각 1분 추이와 함께 표시합니다. 칩이 열로 스로틀링되는 동안에는 해당 카드의 테두리가 빨간색으로 바뀝니다.
- **메모리 및 대역폭**, **네트워크 및 디스크**: 메모리 구성과 압력, 통합 메모리 대역폭, 네트워크·디스크 속도.
- **센서** 및 **프로세스 목록**: 팬 속도와 그룹별 온도 센서. 프로세스 목록은 필터링·정렬할 수 있고, 프로세스를 클릭하면 프로세스별 상세 정보(CPU, IPC, 에너지, 메모리, Neural Engine 메모리)가 열립니다. 종료와 강제 종료에는 프로세스 모니터링과 같은 안전 확인이 적용됩니다.

AI 모니터는 페이지가 화면에 보이는 동안에만 샘플링하며, 모니터링을 일시 중지하면 멈춥니다. 일부 칩은 메모리 대역폭 합계만 제공하므로 이 경우 CPU / GPU / 미디어 항목은 0으로 표시됩니다.
